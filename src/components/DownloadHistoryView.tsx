...
import { History, Download, Trash2, ShieldCheck, Search, AlertCircle, CheckCircle2, Clock } from 'lucide-react';
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in" id="download-history-view">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-6 border-b border-slate-200 dark:border-white/10">
        <div>
          <BackButton onBack={onBack} label={language === 'id' ? 'Kembali' : 'Back'} showText={true} className="mb-3" />
          <h1 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-3">
            <History className="w-7 h-7 text-blue-600 dark:text-blue-400" />
            <span>{language === 'id' ? 'Riwayat Unduhan' : 'Download History'}</span>
          </h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            {language === 'id' 
              ? 'Daftar berkas APK dan tautan resmi aplikasi yang pernah Anda unduh di platform Mod Station.'
              : 'List of APK files and official links of apps you have downloaded on Mod Station.'}
          </p>
        </div>
        ...
        {records.length > 0 && (
          <div className="flex items-center gap-2 self-start sm:self-auto">
            ...
            >
              ...
              <span>{language === 'id' ? 'Bersihkan Riwayat' : 'Clear History'}</span>
            </button>
          </div>
        )}
      </div>
      ...
      {records.length > 0 && (
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 mb-6">
          <div className="relative flex-1 max-w-sm">
            ...
            <input
              type="text"
              placeholder={language === 'id' ? 'Cari dalam riwayat...' : 'Search history...'}
              value={searchQuery}
          ...
          <div className="flex bg-slate-100 dark:bg-white/5 p-1 rounded-xl border border-slate-200 dark:border-white/10 text-xs font-bold">
            ...
            >
              {language === 'id' ? 'Semua' : 'All'} ({records.length})
            </button>
            <button
            ...
            >
              {language === 'id' ? 'Berkas APK' : 'APK Files'} ({records.filter(h => h && h.type === 'apk').length})
            </button>
            <button
            ...
            >
              {language === 'id' ? 'Tautan Resmi' : 'Official Links'} ({records.filter(h => h && h.type === 'official_link').length})
            </button>
          </div>
        </div>
      )}
      ...
      {filteredHistory.length === 0 ? (
        <div className="py-20 text-center max-w-md mx-auto px-4 bg-white dark:bg-white/[0.02] border border-dashed border-slate-200 dark:border-white/10 rounded-3xl">
          ...
          <h3 className="text-lg font-black text-slate-900 dark:text-white mb-2">
            {searchQuery 
              ? (language === 'id' ? 'Tidak ada riwayat yang cocok.' : 'No matching history.')
              : (language === 'id' ? 'Belum ada riwayat unduhan.' : 'No download history yet.')}
          </h3>
          <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed mb-6">
            {searchQuery
              ? (language === 'id' ? 'Coba gunakan kata kunci pencarian nama aplikasi yang lain.' : 'Try using another app name search keyword.')
              : (language === 'id' 
                  ? 'Ketika Anda mengunduh paket APK atau mengunjungi situs resmi aplikasi, riwayat Anda akan tercatat di sini secara privat.'
                  : 'When you download APK packages or visit official app sites, your history will be recorded here privately.')}
          </p>
          {onBackHome && (
            <button
              onClick={onBackHome}
            ...
            >
              {language === 'id' ? 'Jelajahi Aplikasi Sekarang' : 'Explore Apps Now'}
            </button>
          )}
        </div>
      ) : (
        <div className="space-y-3">
          {filteredHistory.map((item) => {
            ...
            const dateStr = new Date(item.downloadedAt).toLocaleString(language === 'id' ? 'id-ID' : 'en-US', {
              day: 'numeric',
              month: 'short',
              year: 'numeric',
              hour: '2-digit',
            ...
            return (
              ...
              >
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold ${
                        ...
                        {item.type === 'apk' 
                          ? (language === 'id' ? 'APK Langsung' : 'Direct APK') 
                          : (language === 'id' ? 'Tautan Resmi' : 'Official Link')}
                      </span>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-[11px] text-slate-400 font-medium">
                      ...
                      {item.fileSize && <span>• {language === 'id' ? 'Ukuran' : 'Size'}: {item.fileSize}</span>}
                      <span className="flex items-center gap-1 text-emerald-600 dark:text-emerald-400 font-bold">
                        <CheckCircle2 className="w-3.5 h-3.5" />
                        <span>{item.status === 'completed' ? (language === 'id' ? 'Selesai' : 'Completed') : (language === 'id' ? 'Dimulai' : 'Started')}</span>
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 self-end sm:self-center">
                  ...
                  >
                    {language === 'id' ? 'Detail Aplikasi' : 'App Details'}
                  </button>

                  {matchedApp && (
                    <button
                      onClick={(e) => handleDownload(e, matchedApp)}
                    ...
                    >
                      ...
                      <span>{language === 'id' ? 'Unduh Lagi' : 'Download Again'}</span>
                    </button>
                  )}
                </div>
              </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl">
            ...
            <div className="space-y-2">
              <h3 className="text-lg font-black text-slate-900 dark:text-white">
                {language === 'id' ? 'Bersihkan Semua Riwayat Unduhan?' : 'Clear All Download History?'}
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed font-medium">
                {language === 'id'
                  ...
                  : 'This action will permanently delete all app download activity records from your device and account database.'}
              </p>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              ...
              >
                {language === 'id' ? 'Ya, Bersihkan Riwayat' : 'Yes, Clear History'}
              </button>
            </div>
          </div>
        </div>
