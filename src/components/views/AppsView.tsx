...
export default function AppsView({
  ...
  // Filter out games to get pure applications
  const pureApps = useMemo(() => {
    return apps.filter(app => {
      const cat = (app.category || '').toLowerCase().trim();
      const type = ((app as any).type || '').toLowerCase();
      return cat !== 'game' && cat !== 'games' && cat !== 'permainan' && type !== 'game';
    });
  }, [apps]);

  // Extract unique categories from real apps
  ...
  const forYouApps = useMemo(() => {
    return [...pureApps].sort((a, b) => (b.rating || 0) - (a.rating || 0));
  }, [pureApps]);

  // Tab 2: Most Popular
  const popularApps = useMemo(() => {
    return [...pureApps].sort((a, b) => (b.downloads || 0) - (a.downloads || 0));
  }, [pureApps]);

  // Tab 3: New / Recently Updated
  ...
  // Active list based on tab & category filter
  const displayedApps = useMemo(() => {
    let source = activeTab === 'popular' ? popularApps : activeTab === 'new' ? newApps : forYouApps;
    if (selectedCategory !== 'all') {
      source = source.filter(a => (a.category || '').toLowerCase().trim() === selectedCategory.toLowerCase().trim());
    }
    return source;
  }, [activeTab, selectedCategory, forYouApps, popularApps, newApps]);
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-8" id="apps-page-container">
      {/* Title Header */}
      <div className="space-y-1">
        ...
        <p className="text-sm font-medium text-slate-500 dark:text-slate-400">
          Temukan utilitas, produktivitas, dan aplikasi Android terbaik pilihan redaksi.
        </p>
      </div>
      ...
      <div className="flex items-center gap-2 border-b border-slate-200/80 dark:border-white/10 overflow-x-auto no-scrollbar pb-1">
        {[
          { id: 'for-you', label: 'Untuk Anda', icon: Sparkles },
      ...
      {/* Editorial Discovery Shelf ("Aplikasi Pilihan Redaksi") */}
      {selectedCategory === 'all' && forYouApps.length > 0 && (
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold tracking-tight text-slate-900 dark:text-white">
            {activeTab === 'popular' ? 'Aplikasi Terpopuler' : activeTab === 'new' ? 'Baru & Diperbarui' : 'Daftar Aplikasi'}
          </h2>
