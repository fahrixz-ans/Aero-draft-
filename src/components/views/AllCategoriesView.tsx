...
import { 
  Layers, Search, ChevronRight, Sparkles, Download, 
  Grid, ChevronDown, ChevronUp, ArrowRight, Filter
} from 'lucide-react';
import { AppData } from '../../types';
import Breadcrumb from '../Breadcrumb';
import FollowSocialSection from '../FollowSocialSection';
import { getCategoriesWithStats, categoryToSlug } from '../../utils/categoryUtils';
...
export default function AllCategoriesView({
  ...
  // Compute categories and stats dynamically from real Firestore apps
  const allCategories = useMemo(() => {
    return getCategoriesWithStats(apps, customCategories);
  }, [apps, customCategories]);

  // Top categories (by app count & downloads)
  const topCategories = useMemo(() => {
    return allCategories.slice(0, topCatsExpanded ? 12 : 6);
  }, [allCategories, topCatsExpanded]);

  // Filter categories by search query and alphabet
  const filteredCategories = useMemo(() => {
    return allCategories.filter(cat => {
      const matchSearch = cat.name.toLowerCase().includes(searchQuery.toLowerCase().trim()) ||
        cat.description.toLowerCase().includes(searchQuery.toLowerCase().trim());
      if (!matchSearch) return false;

      if (selectedLetter === 'Semua') return true;
      if (selectedLetter === '#') {
        return /^[0-9]/.test(cat.name.trim());
      }
      return cat.name.trim().toUpperCase().startsWith(selectedLetter);
    });
  }, [allCategories, searchQuery, selectedLetter]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-8 animate-fade-in" id="all-categories-container">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 text-xs font-bold border border-blue-500/20">
              ...
              <span>Direktori Lengkap Kategori</span>
            </div>
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-black text-slate-900 dark:text-white tracking-tight">
              Kategori Aplikasi & Games Android
            </h1>
            <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 max-w-2xl">
              Jelajahi seluruh kategori untuk menemukan aplikasi produktivitas, game aksi, media sosial, dan utilitas Android terverifikasi aman.
            </p>
          </div>
          ...
          <div className="relative w-full md:w-80 shrink-0">
            ...
            <input
              ...
              placeholder="Cari kategori aplikasi..."
      ...
      <div className="space-y-4" id="top-categories-section">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Kategori dengan jumlah aplikasi dan aktivitas unduhan tertinggi
              </p>
            </div>
          </div>
          ...
          {allCategories.length > 6 && (
            <button
              onClick={() => setTopCatsExpanded(!topCatsExpanded)}
            ...
            >
              {topCatsExpanded ? (
                ...
                <>
                  <span>Lihat Semua Top ({allCategories.length})</span>
          ...
          {topCategories.map((cat) => (
            ...
            >
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-blue-600 to-cyan-600 text-white flex items-center justify-center font-black text-xl shadow-md shrink-0 group-hover:scale-105 transition-transform">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400 mt-2">
                    ...
                    <span>·</span>
                    <span>
                      {formatDownloadCount(cat.totalDownloads)} Unduhan
                    </span>
                  </div>
                </div>
              ...
              {/* Preview top 2 apps */}
              {cat.topApps.length > 0 && (
                <div className="pt-3 border-t border-slate-100 dark:border-white/5 flex items-center gap-2 overflow-hidden">
                  <span className="text-[10px] text-slate-400 font-bold shrink-0">Populer:</span>
                  <div className="flex items-center gap-1.5 min-w-0 truncate">
                    {cat.topApps.slice(0, 2).map(app => (
                      <span
                        key={app.id}
      ...
      <div className="space-y-4" id="all-categories-alphabet-section">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Ditemukan {filteredCategories.length} kategori yang tersedia
            </p>
          </div>
        </div>
        ...
        {paginatedCategories.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {paginatedCategories.map(cat => (
              ...
              >
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">
                          {cat.count} Aplikasi · {formatDownloadCount(cat.totalDownloads)} Unduhan
                        </p>
                      </div>
                    </div>
                ...
                >
                  <span>Buka Kategori ({cat.count} Aplikasi)</span>
          ...
          <div className="p-12 text-center bg-white dark:bg-[#131924] rounded-2xl border border-slate-200/80 dark:border-white/10 space-y-2">
            ...
            <p className="text-xs text-slate-500">
              Silakan coba kata kunci lain atau pilih abjad 'Semua'.
            </p>
            <button
              onClick={() => {
